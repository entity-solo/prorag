...
## Answer"""
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # SIÊU NĂNG LỰC 4: THÌ THỜI GIAN & CHUẨN HÓA CÂU BỊ ĐỘNG (TEMPORAL ASPECT & PASSIVE NORMALIZATION)
    # ──────────────────────────────────────────────────────────────────────────
    print_banner("SUPERPOWER 4: TEMPORAL ASPECT REASONING & PASSIVE VOICE NORMALIZATION")

    prorag_temporal = ProRAG(model=MODEL)

    print("Ingesting passive voice statement...")
    # "Project Apollo was developed by NASA."
    # The new passive normalization rule will convert this to active: nasa -> developed -> project apollo
    prorag_temporal.ingest("Project Apollo was developed by NASA.", source="History Archive")

    print("\nIngesting event with specific time aspect (FUTURE plan)...")
    # "In May 2026, NASA stated that it plans to launch Apollo 11 next year."
    # This will extract statement_time="May 2026", condition="next year", temporal_aspect="FUTURE"
    prorag_temporal.ingest("In May 2026, NASA stated that it plans to launch Apollo 11 next year.", source="Press Release")

    print("\n--- Examining Graph Nodes and Edges ---")
    print(f"Graph stats: {prorag_temporal.stats()}")
    for u, v, data in prorag_temporal.graph.g.edges(data=True):
        meta = data["meta"]
        print(f"Edge: {u} -> ({data['relation']}) -> {v}")
        print(f"  Metadata: condition='{meta.condition}', negated={meta.negated}, statement_time='{meta.statement_time}', temporal_aspect='{meta.temporal_aspect}'")

    print("\nQuerying about plans:")
    q_plan = "What are the future plans for NASA?"
    ans_plan = prorag_temporal.ask(q_plan)
    print(f"Question: '{q_plan}'")
    print(f"ProRAG Answer: {ans_plan['answer']}")
