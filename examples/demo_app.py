"""
Streamlit demo — upload documents, ask questions, see graph stats.
...
st.set_page_config(page_title="ProRAG Demo", page_icon="🧠", layout="wide")
st.title("🧠 ProRAG — Proactive Knowledge Graph RAG")
st.caption("Upload documents → ask questions → zero hallucinations from known facts")
...
with st.sidebar:
    ...
    if st.button("🗑️ Clear graph"):
        from prorag import ProRAGGraph
        st.session_state.rag = ProRAG()
        st.session_state.history = []
        st.rerun()
...
with col1:
    ...
    for item in reversed(st.session_state.history):
        ...
        with st.container(border=True):
            ...
            cols = st.columns(3)
            cols[0].caption(f"Domains: {', '.join(r['domains'])}")
            cols[1].caption(f"Triples used: {r['triples_used']}")
            if r["sources"]:
                cols[2].caption(f"Sources: {', '.join(r['sources'])}")
            if r["has_contradictions"]:
                st.warning("⚠️ Conflicting information exists in the graph for this topic.")
